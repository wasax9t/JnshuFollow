...
//		hit.setSecure(true);  //如果设置了Secure，则只有当使用https协议连接时cookie才可以被页面访问
	...
	public ModelAndView addCookie(HttpServletResponse response){
		ModelAndView mAndView = new ModelAndView("cookieShow");

//		CookieUtil.addCookie(response, "test_addCookie", UUID.randomUUID().toString(), null);
		CookieUtil.addCookie(response, "test_2", UUID.randomUUID().toString() + new Random(1000).nextInt(), "localhost");
	...
	public void getCookie(HttpServletRequest request){
		System.out.println(CookieUtil.getCookieValue(request, "test_2"));
	}
	
	@RequestMapping("/testEditCookie")
	public ModelAndView editCookie(HttpServletRequest request,HttpServletResponse response){
		ModelAndView mAndView = new ModelAndView("cookieShow");
		
		CookieUtil.editCookie(request, response, "test_2", "editeditedit", "localhost");
		
		return mAndView;
	}
	
	@RequestMapping("/testDelCookie")
	public void delCookie(HttpServletRequest request,HttpServletResponse response){
		CookieUtil.delCookie(request, response, "test_2");
	}
}
